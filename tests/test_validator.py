...
class TestValidator(unittest.TestCase):
    ...
    def test_valid_message(self):
        ...
        record = "+:42"
        msg = DummyMsg(sender, sessionId, startTime, record, [])
        # Simulate valid signature chain
        sig = self.sigMan.sign(msg, self.peer_keys[sender])
        msg.signatures = [[self.peer_certs[sender], sig]]
        validator = Validator(self.recordPattern, self.ca, self.sigMan)
        result = validator.validate(msg, self.roundTW, self.node_id, self.peers)
